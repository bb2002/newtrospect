# Eval 3-articles [round1] — 2026-05-22T01:09:47.548Z
...
### 🟡 context
- model: gemini-3.1-flash-lite-preview, 1411ms
- matched=2/4 (recall 50%) · api=3 (precision 67%)
- matched:
  - REF `문제는 CPU 위주의 작업이 아니었습니다.` ↔ API `문제는 CPU 위주의 작업이 아니었습니다.`
...
### 🔴 sensational
- model: gemini-3.1-flash-lite-preview, 1426ms
- matched=1/1 (recall 100%) · api=4 (precision 25%)
- matched:
  - REF `하지만 여기에는 보존 정책이라는 치명적인 결함이 있었습니다.` ↔ API `하지만 여기에는 보존 정책이라는 치명적인 결함이 있었습니다.`
- extra (정답에 없는데 API 가 추가로 뽑음):
  - 작업은 점점 더 느려지고 있었고, 기한이 임박했습니다.
  - 수만 개의 부분과 수백 개의 동시 쿼리를 통해 모든 작업이 하나의 파일 줄에 서 있었습니다.
  - 결과: 쿼리 지속 시간이 즉각적으로 엄청나게 줄어듭니다.

### 🔵 term
- model: gemini-3.1-flash-lite-preview, 1984ms
- matched=8/10 (recall 80%) · api=9 (precision 89%)
- matched:
  - REF `OLAP` ↔ API `OLAP`
  - REF `TTL` ↔ API `TTL`
  - REF `뮤텍스` ↔ API `뮤텍스`
...
### 🟢 quantitative
- model: gemini-3.1-flash-lite-preview, 1733ms
...
### 🟡 context
- model: gemini-3.1-flash-lite-preview, 1177ms
...
### 🔴 sensational
- model: gemini-3.1-flash-lite-preview, 1367ms
...
### 🔵 term
- model: gemini-3.1-flash-lite-preview, 2313ms
...
### 🟢 quantitative
- model: gemini-3.1-flash-lite-preview, 1413ms
...
### 🟡 context
- model: gemini-3.1-flash-lite-preview, 998ms
- matched=1/3 (recall 33%) · api=3 (precision 33%)
- matched:
...
- missed (정답에 있는데 API 가 못 뽑음):
  ...
  - 시뮬레이션 결과 이 직원은 총 2억 4719만원을 근로소득세로 내야 하는 것으로 나타났다.
- extra (정답에 없는데 API 가 추가로 뽑음):
  - 여기에 지방소득세를 더하게 되면 최종 세액은 2억 7000만원대로 추정된다.
  - 이 중 회사가 2억 6000여만원을 원천징수하고 근로자는 700만~800만원 정도를 직접 내야 할 것으로 분석됐다.

### 🔴 sensational
- model: gemini-3.1-flash-lite-preview, 890ms
...
### 🔵 term
- model: gemini-3.1-flash-lite-preview, 2104ms
- matched=8/9 (recall 89%) · api=8 (precision 100%)
- matched:
...
### 🟢 quantitative
- model: gemini-3.1-flash-lite-preview, 2186ms
- matched=13/13 (recall 100%) · api=14 (precision 93%)
- matched:
...
|---|---|---|---|---|---|---|
| 🟡 context | 10 | 9 | 5 | 56% | 50% | 0.53 |
| 🔴 sensational | 5 | 8 | 5 | 63% | 100% | 0.77 |
| 🔵 term | 25 | 25 | 21 | 84% | 84% | 0.84 |
| 🟢 quantitative | 29 | 30 | 29 | 97% | 100% | 0.98 |

## Per-article matched/recall

| article | 🟡 context | 🔴 sensational | 🔵 term | 🟢 quantitative |
|---|---|---|---|---|
| b1-clickhouse | 2/4 (api 3) | 1/1 (api 4) | 8/10 (api 9) | 10/10 (api 10) |
| b2-tumen | 2/3 (api 3) | 3/3 (api 3) | 5/6 (api 8) | 6/6 (api 6) |
| b3-samsung-tax | 1/3 (api 3) | 1/1 (api 1) | 8/9 (api 8) | 13/13 (api 14) |